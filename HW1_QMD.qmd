...
```{r}
top_localities <- head(arrange(commune_level_data, desc(n_offers)), 10)

# Çubuk grafik oluştur
library(ggplot2)
ggplot(data = top_localities, aes(x = locality, y = n_offers, fill = locality)) +
  geom_bar(stat = "identity") +
  labs(title = "En Çok Teklif Alan Top 10 Locality", x = "Locality", y = "Teklif Sayısı") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1) ) +
  scale_fill_manual(values = rainbow(nrow(top_localities)))
```
